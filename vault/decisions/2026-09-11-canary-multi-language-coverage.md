...
Dogfooding against the real target repos (`acme-sls`, Python; `acme-extension`, Node) found `canary init` fails immediately outside a Go module — the Go-only v1 scope was deliberate but left the tool unusable on two of the three repos it was built to serve.
...
- Full session narrative, every finding, and every ruling made along the way: `acme-site/vault/sessions/2026-09-11-canary.md`.
...
- Every fix (including the follow-up) was verified red-then-green against reverted code before being restored, plus an independent code review. Session narrative: `acme-site/vault/sessions/2026-09-12-canary.md`.
